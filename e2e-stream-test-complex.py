#!/usr/bin/env python3
"""E2E test for AuroraCraft real-time SSE streaming with a complex prompt."""
import requests
import threading
import time
import json
            ...
            if line and line.startswith("data: "):
                ...
                try:
                    ...
                    if etype == "status" and event.get("status") == "connected":
                        ...
                        print(f"  [{time.strftime('%H:%M:%S', time.localtime(ts))}.{int(ts*1000)%1000:03d}] TEXT #{text_chunk_count}: {preview}")
                    elif etype == "thinking":
                        done = event.get("done", False)
                        content = event.get("content", "")
                        label = "THINK-DONE" if done else "THINKING"
                        print(f"  [{time.strftime('%H:%M:%S', time.localtime(ts))}.{int(ts*1000)%1000:03d}] {label}: {repr(content[:80])}")
                    elif etype == "file-op":
                        ...
                        print(f"  [{time.strftime('%H:%M:%S', time.localtime(ts))}.{int(ts*1000)%1000:03d}] ERROR: {event.get('message','')[:200]}")
                        sse_done.set()
                    elif etype == "status":
                        print(f"  [{time.strftime('%H:%M:%S', time.localtime(ts))}.{int(ts*1000)%1000:03d}] STATUS: {event.get('status','')}")
                    else:
                        print(f"  [{time.strftime('%H:%M:%S', time.localtime(ts))}.{int(ts*1000)%1000:03d}] {etype}")
...
# Send complex message that should generate substantial output
PROMPT = "Create a complete Random Teleport Plugin for Paper 1.21. Include: build.gradle with paper-api dependency, plugin.yml with rtp command, main class that registers the command, and a command executor that teleports the player to a random location within 1000 blocks. Use proper Bukkit API."

print(f"=== Sending complex prompt ({len(PROMPT)} chars) ===")
send_time = time.time()
r = sess.post(
...
if text_events:
    ...
    if len(text_events) > 1:
        ...
        if spread > 0.5:
            ...
            print(f"\n⚠ Text arrived in {len(text_events)} chunks but in {spread:.3f}s (may be micro-buffered by model)")
    else:
        print(f"\n⚠ Only 1 text-delta event (model may have returned all text at once)")
else:
    print(f"\n✗ NO text-delta events received!")
    if error_events:
        for e in error_events:
            print(f"  Error: {str(e['event'].get('message',''))[:300]}")
    print(f"  Total time: {end_time - send_time:.3f}s")

if thinking_events:
    think_times = [e["time"] for e in thinking_events]
    think_spread = max(think_times) - min(think_times) if len(think_times) > 1 else 0
    print(f"✓ Thinking: {len(thinking_events)} events over {think_spread:.1f}s")
if fileop_events:
    print(f"✓ File ops: {len(fileop_events)} operations streamed")
    for e in fileop_events:
        ev = e["event"]
        print(f"    {ev.get('action','')} {ev.get('path','')} [{ev.get('status','')}]")
if complete_events:
